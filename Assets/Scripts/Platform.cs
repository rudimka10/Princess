...
public class Platform : MonoBehaviour
{
    ...
    {
        if (_attachPlayer && c.gameObject.TryGetComponent(out Player player))
            player.transform.parent = _localTransform;
    }

    private void OnCollisionExit2D(Collision2D c)
    {
        if (_attachPlayer && c.gameObject.TryGetComponent(out Player player) && player.transform.parent == _localTransform)
